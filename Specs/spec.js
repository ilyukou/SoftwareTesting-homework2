...
describe("Tag manager homepage test", function () {
    ...
    it('2', async function(){
       ...
       inputNameAccount.sendKeys(data.company.name);
    
       expect(labelNameAccount.getText()).toEqual('Название аккаунта');
       expect(inputNameAccount.getAttribute('value')).toEqual(data.company.name);
    });
    it('3', async function(){
        browser.sleep(9000);
    ...
    it('5', function (){
        ...
        inputFormSettingContainer.sendKeys(data.company.name);

        expect(inputFormSettingContainer.getAttribute('value')).toEqual(data.company.name);
